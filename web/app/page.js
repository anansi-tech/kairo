    ...
    <>
      ...
      >
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse 60% 50% at 50% 30%, rgba(214,155,50,0.10), transparent 70%)",
          }}
        />
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4 animate-fade-up animate-fade-up-delay-3">
            <a
              ...
              className="inline-flex items-center gap-2 px-8 py-4 bg-anansi-bronze text-anansi-cream
                         ...
                         hover:bg-anansi-espresso transition-colors min-w-[260px] justify-center"
            >
              Get notified about the next cohort →
            </a>
            <a
              href="#what-you-learn"
              className="text-[13px] tracking-[0.1em] uppercase text-anansi-espresso/65
                         hover:text-anansi-bronze transition-colors"
            >
              See what you learn
            </a>
          </div>
      ...
      >
        <div className="max-w-[1200px] mx-auto reveal">
          ...
          <p className="mt-7 text-[17px] text-anansi-espresso/75 leading-[1.75] max-w-[760px]">
      ...
      <section className="py-24 md:py-28 px-6 md:px-12 border-t border-anansi-espresso/10">
        <div className="max-w-[1200px] mx-auto reveal">
          ...
          <h2 className="font-display text-[clamp(28px,3.6vw,44px)] leading-[1.15] text-anansi-espresso max-w-[760px]">
            ...
            <br />
            <span className="italic text-anansi-bronze">Built by an operator.</span>
          </h2>
      ...
      <section className="py-24 md:py-28 px-6 md:px-12 border-t border-anansi-espresso/10 bg-anansi-espresso text-anansi-cream">
        <div className="max-w-[1200px] mx-auto reveal">
          ...
          <h2 className="font-display text-[clamp(28px,3.6vw,44px)] leading-[1.15] max-w-[700px]">
            Small cohorts. <span className="italic text-anansi-gold">Premium price.</span>
          </h2>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-anansi-cream/10 mt-14">
        ...
        <div className="max-w-[1200px] mx-auto grid md:grid-cols-[280px_minmax(0,1fr)] gap-12 md:gap-16 items-start reveal">
          <div className="flex flex-col items-center md:items-start">
            <div className="w-[200px] h-[200px] md:w-[240px] md:h-[240px] rounded-full bg-anansi-cream border-2 border-anansi-gold shadow-[0_24px_48px_rgba(42,26,18,0.18)] flex items-center justify-center">
              ...
              <span className="font-display text-[64px] md:text-[80px] text-anansi-bronze leading-none">
                DN
              </span>
            </div>
            <p className="mt-5 text-[12px] tracking-[0.18em] uppercase text-anansi-espresso/60">
              Your instructor
            </p>
          </div>
          ...
          <div>
            ...
            <h2 className="font-display text-[clamp(28px,3.6vw,44px)] leading-[1.15] text-anansi-espresso">
              David Noel
            </h2>
            <p className="mt-5 text-[16px] text-anansi-espresso/80 leading-[1.8]">
      ...
      <section
        id="cta"
        className="py-28 md:py-32 px-6 md:px-12 border-t border-anansi-espresso/10 bg-anansi-cream relative overflow-hidden"
      >
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
        ...
        <div className="max-w-[760px] mx-auto text-center relative reveal">
          ...
          <p className="mt-6 text-[17px] text-anansi-espresso/75 leading-[1.7] max-w-[580px] mx-auto">
...
function Eyebrow({ children, center }) {
  return (
    <div className={`flex items-center gap-3 mb-6 ${center ? "justify-center" : ""}`}>
      <span className="block w-8 h-px bg-anansi-bronze" />
      <p className="text-[10px] tracking-[0.25em] uppercase text-anansi-bronze font-semibold">
        {children}
      </p>
    </div>
  );
...
function DayCard({ num, title, items }) {
  return (
    <div className="border-t border-anansi-espresso/15 pt-7">
      <p className="text-[11px] tracking-[0.2em] uppercase text-anansi-bronze font-semibold">
        {num}
      </p>
      <h3 className="font-display text-[26px] md:text-[30px] leading-[1.2] text-anansi-espresso mt-3">
        {title}
      </h3>
      <ul className="mt-7 space-y-4">
        {items.map((item, i) => (
          <li key={i} className="flex gap-3 items-baseline">
            <span className="text-anansi-bronze text-[11px] font-mono shrink-0 mt-[3px]">
              {String(i + 1).padStart(2, "0")}
            </span>
            <p className="text-[15px] text-anansi-espresso/85 leading-[1.65]">{item}</p>
          </li>
        ))}
      </ul>
    </div>
...
function Persona({ role, quote }) {
  return (
    <div className="border border-anansi-espresso/12 bg-anansi-cream p-7 rounded-sm">
      <p className="text-[10px] tracking-[0.18em] uppercase text-anansi-bronze font-semibold mb-4">
        {role}
      </p>
      <p className="text-[15px] text-anansi-espresso/85 leading-[1.65] italic">
        &ldquo;{quote}&rdquo;
      </p>
    </div>
  );
...
function Moat({ title, children }) {
  return (
    <div>
      <h3 className="font-display text-[22px] md:text-[24px] text-anansi-espresso italic">
        {title}
      </h3>
      <p className="mt-3 text-[15px] text-anansi-espresso/80 leading-[1.75]">{children}</p>
    </div>
  );
}
...
function Stat({ label, value, sub }) {
  return (
    <div className="bg-anansi-espresso p-6 md:p-7">
      <p className="text-[10px] tracking-[0.18em] uppercase text-anansi-gold/70 mb-3">{label}</p>
      <p className="font-display text-[clamp(28px,3.4vw,40px)] leading-none text-anansi-cream">
        {value}
      </p>
      <p className="text-[11px] tracking-[0.1em] uppercase text-anansi-cream/55 mt-3">{sub}</p>
    </div>
  );
}
...
function PriceTile({ primary, label }) {
  return (
    <div className="border border-anansi-cream/15 px-6 py-6 rounded-sm">
      <p className="font-display text-[28px] md:text-[32px] text-anansi-gold leading-none">
        {primary}
      </p>
      <p className="text-[12px] tracking-[0.08em] uppercase text-anansi-cream/70 mt-3">{label}</p>
    </div>
  );
}
